...
def load_static_data(dataset: AdforceLazyDataset) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def calculate_global_limits(
    ...
    for v in vars_to_scan:
        ...
        if v in diverging_vars:
            ...
            if abs_max == 0: abs_max = 0.1
            limits[v] = (-abs_max, abs_max)
        else:
            if g_min == g_max: g_max += 0.1
...
def plot_delta_frame(
    ...
    titles = [
        ...
        [r"$\Delta$ $\hat{\text{SSH}}$ [m]", r"$\Delta$ $\hat{\text{VX}}$ [m s$^{-1}$]", r"$\Delta$ $\hat{\text{VY}}$ [m s$^{-1}$]"]
    ]
